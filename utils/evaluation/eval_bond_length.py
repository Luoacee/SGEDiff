...
def get_bond_length_profile(bond_lengths: Sequence[BondLengthData]) -> BondLengthProfile:
    ...
    for bond_type, bond_length in bond_lengths:
        ...
        bond_length_profile[bond_type].append(bond_length)  # (s, e, bcls)
...
def get_pair_length_profile(pair_lengths):  # 分析键长分布范围
    cc_dist = [d[1] for d in pair_lengths if d[0] == (6, 6) and d[1] < 2]  # 碳碳距离小于2
    all_dist = [d[1] for d in pair_lengths if d[1] < 12]  # 计数总距离
    pair_length_profile = {
        'CC_2A': get_distribution(cc_dist, bins=np.linspace(0, 2, 100)),  # 划分盒子，分析键长分布
...
def eval_pair_length_profile(pair_length_profile):
    metrics = {}
    for k, gt_distribution in eval_bond_length_config.PAIR_EMPIRICAL_DISTRIBUTIONS.items():  # 经验列表
        if k not in pair_length_profile:
            metrics[f'JSD_{k}'] = None
        else:
            metrics[f'JSD_{k}'] = sci_spatial.distance.jensenshannon(gt_distribution, pair_length_profile[k])  # CC2A 12A
    return metrics  # JSD_CC_2A JSD_All_12A
