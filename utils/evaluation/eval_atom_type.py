...
def eval_atom_type_distribution(pred_counter: Counter):  # 评估原子类型
    total_num_atoms = sum(pred_counter.values())  # 统计原子总数
    pred_atom_distribution = {}
    for k in ATOM_TYPE_DISTRIBUTION:
        pred_atom_distribution[k] = pred_counter[k] / total_num_atoms  # 原子类型占比
