...
from models.SGEDiff.common import compose_context, ShiftedSoftplus, compose_context_single
from models.SGEDiff.uni_transformer import SGEDiff
...
def log_sample_categorical(logits):  
    ...
    gumbel_noise = -torch.log(-torch.log(uniform + 1e-30) + 1e-30)  # log
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):  
        ...
        self.alphas_cumprod = to_torch_const(alphas_cumprod) # alphas_cumprod
        self.alphas_cumprod_prev = to_torch_const(alphas_cumprod_prev) # alphas_cumprod_prev
    ...
    def forward(self, protein_pos, protein_v, batch_protein, subgraph_info, init_ligand_pos, init_ligand_v,
        ...
        h_all, pos_all, batch_all, mask_ligand, h_imp, pos_imp, batch_imp, imp_bonds, imp_cls, imp_mask = compose_context(  
            h_protein=h_protein,  
        ...
        subgraph = dict(h=h_imp, pos=pos_imp, batch=batch_imp, mask=imp_mask, bonds=imp_bonds, cls=imp_cls)

        outputs = self.refine_net(h_all, pos_all, mask_ligand, batch_all, subgraph, return_all=return_all, fix_x=fix_x) 
        final_pos, final_h, mask_ligand = outputs['x'], outputs['h'], outputs["mask"]
    ...
    def forward_for_sample(self, protein_pos, protein_v, batch_protein, subgraph_info, init_ligand_pos, init_ligand_v,
                batch_ligand, time_step=None, return_all=False, fix_x=False, origin_ligand_pos=None,
                           origin_ligand_v=None, origin_batch_ligand=None):  
        
        batch_size = batch_protein.max().item() + 1  
        if len(init_ligand_v.shape) == 1:  
            init_ligand_v = F.one_hot(init_ligand_v, self.num_classes).float()
            if origin_ligand_v is not None:
                origin_ligand_v = F.one_hot(origin_ligand_v, self.num_classes).float()
        elif (origin_ligand_v is not None) and len(init_ligand_v.shape) == 2 and len(origin_ligand_v.shape) ==2:
            pass
        else:
            raise ValueError
        # time embedding
        if self.time_emb_dim > 0:  
            if self.time_emb_mode == 'simple':  
                ...
                if origin_ligand_v is not None:
                    input_origin_ligand_feat = torch.cat([
                        origin_ligand_v,
                        (time_step / self.num_timesteps)[origin_batch_ligand].unsqueeze(-1)
                    ], -1)
            elif self.time_emb_mode == 'sin': 
                time_feat = self.time_emb(time_step)
                input_ligand_feat = torch.cat([init_ligand_v, time_feat], -1)
                if origin_ligand_v is not None:
                    input_origin_ligand_feat = torch.cat([origin_ligand_v, time_feat], -1)
            else:
                raise NotImplementedError
        else:  
            input_ligand_feat = init_ligand_v
            if origin_ligand_v is not None:
                input_origin_ligand_feat = origin_ligand_v

        h_protein = self.protein_atom_emb(protein_v)  
        init_ligand_h = self.ligand_atom_emb(input_ligand_feat)  
        if origin_ligand_v is not None:
            origin_h_ligand = self.ligand_atom_emb(input_origin_ligand_feat)

        if self.config.node_indicator:  
            h_protein = torch.cat([h_protein, torch.zeros(len(h_protein), 1).to(h_protein)], -1)
            init_ligand_h = torch.cat([init_ligand_h, torch.ones(len(init_ligand_h), 1).to(h_protein)], -1)
            if origin_ligand_v is not None:
                origin_h_ligand = torch.cat([origin_h_ligand, torch.ones(len(origin_h_ligand), 1).to(h_protein)], -1)

        if origin_ligand_v is not None:
            _, _, _, _, h_imp, pos_imp, batch_imp, imp_bonds, imp_cls, imp_mask = compose_context(  
                h_protein=h_protein,  
                h_ligand=origin_h_ligand,  
                pos_protein=protein_pos,  
                pos_ligand=origin_ligand_pos, 
                batch_protein=batch_protein,  
                batch_ligand=origin_batch_ligand,  
                subgraph_info=subgraph_info,
            )

            subgraph_info = dict(h=h_imp, pos=pos_imp, batch=batch_imp, imp_mask=imp_mask, imp_bonds=imp_bonds,
                                 cls=imp_cls)
        else:

            subgraph_info = dict(h=None, pos=None, batch=None, imp_mask=None, imp_bonds=None,
                                 cls=[])


        h_all, pos_all, batch_all, mask_ligand, _, _, _, _, _, _ = compose_context(  
            h_protein=h_protein,  
            h_ligand=init_ligand_h,  
            pos_protein=protein_pos,  
            pos_ligand=init_ligand_pos, 
        ...
        outputs = self.refine_net(h_all, pos_all, mask_ligand, batch_all, subgraph_info, return_all=return_all, fix_x=fix_x) 
        final_pos, final_h, mask_ligand = outputs['x'], outputs['h'], outputs["mask"]
    ...
    def get_diffusion_loss(  
            self, protein_pos, protein_v, affinity, batch_protein, subgraph_info, ligand_pos, ligand_v, batch_ligand, time_step=None
    ):  
        ...
        if time_step is None:  
            ...
            pt = torch.ones_like(time_step).float() / self.num_timesteps  # XXXXXXX
        ...
        ligand_pos_perturbed = a_pos.sqrt() * ligand_pos + (1.0 - a_pos).sqrt() * pos_noise  # pos_noise * std t？
        # Vt = a * V0 + (1-a) / K
    ...
    def pv_joint_guide_cross(self, ligand_v_index, ligand_pos, protein_v, protein_pos, batch_protein,
                       batch_ligand, subgraph_info, origin_ligand_pos=None,
                           origin_ligand_v=None, origin_batch_ligand=None):  
        with torch.enable_grad():
            ligand_v = F.one_hot(ligand_v_index, self.num_classes).float().detach().requires_grad_(
                True)  
            ...
            if origin_ligand_v is not None:
                origin_v = F.one_hot(origin_ligand_v, self.num_classes).float().detach().requires_grad_(
                    True)  
                init_origin_ligand_h = self.ligand_atom_emb(origin_v)
                origin_ligand_pos = origin_ligand_pos.detach().requires_grad_(True)
                origin_ligand_h = torch.cat(
                    [init_origin_ligand_h, torch.ones(len(init_origin_ligand_h), 1).to(init_ligand_h)], -1)

            h_protein = torch.cat([init_h_protein, torch.zeros(len(init_h_protein), 1).to(init_h_protein)],
                                  -1)  
            ligand_h = torch.cat([init_ligand_h, torch.ones(len(init_ligand_h), 1).to(init_ligand_h)], -1)  
            ...
            if origin_ligand_v is not None:
                ...
                subgraph = dict(h=None, pos=None, batch=None, mask=None, bonds=None, cls=[])
            ...
            outputs = self.refine_net(h_all, pos_all, mask_ligand, batch_all, subgraph)  
            final_pos, final_h, mask_ligand = outputs['x'], outputs['h'], outputs["mask"]
            ...
            type_grad = torch.autograd.grad(pred_affinity, ligand_v, grad_outputs=torch.ones_like(pred_affinity),
                                            retain_graph=True)[0]  
            pos_grad = torch.autograd.grad(pred_affinity_log, ligand_pos, grad_outputs=torch.ones_like(pred_affinity),
                                           retain_graph=True)[0]  
    ...
    def sample_diffusion(self, guide_mode, type_grad_weight, pos_grad_weight, protein_pos, protein_v, batch_protein,
                         ...
                         num_steps=None, center_pos_mode=None, origin_ligand_pos=None, origin_ligand_v=None,
                         origin_batch_ligand=None):
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):  
            ...
            if guide_mode in ["gic_score", "gic_score_none"]:
                preds, type_grad, pos_grad = self.pv_joint_guide_cross(ligand_v, ligand_pos, protein_v, protein_pos,
                                                                       batch_protein,
                                                                       batch_ligand, subgraph_info,
                                                                       origin_ligand_pos,
                                                                       origin_ligand_v,
                                                                       origin_batch_ligand
                                                                       )

            elif guide_mode in ["gic_no_score", "gic_no_score_none"]:
                preds = self.forward_for_sample(
                    protein_pos=protein_pos,  
                    protein_v=protein_v,  
                    batch_protein=batch_protein,  
                    subgraph_info=subgraph_info,
                    ...
                    batch_ligand=batch_ligand,  
                    time_step=t,  
                    origin_batch_ligand = origin_batch_ligand,
                    origin_ligand_pos = origin_ligand_pos,
                    origin_ligand_v = origin_ligand_v
                )  
            ...
            elif guide_mode in ["gic_score_none", "gic_score"]:
                exp_pred = preds['final_exp_pred']  

                pos_model_mean = pos_model_mean + pos_grad_weight * (
                            0.5 * pos_log_variance).exp() * pos_grad  
                log_ligand_v = log_ligand_v + type_grad_weight * type_grad  
            ...
            elif guide_mode in ["gic_no_score_none", "gic_no_score"]:
                ligand_pos_next = pos_model_mean + nonzero_mask * (0.5 * pos_log_variance).exp() * torch.randn_like(
                    ligand_pos)
                ligand_pos = ligand_pos_next
                log_model_prob = self.q_v_posterior(log_ligand_v_recon, log_ligand_v, t, batch_ligand)
